...
import { sendBookingNotification, sendCustomerConfirmation } from '../utils/email';
...
const BookingModal = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const templateParams = {
        ...
        is_custom_booking: selectedEvent?.isCustom ? "Yes" : "No"
      };
  ...
  // Format date without time for custom events
        ...
        <div className="relative bg-mystic-navy border border-white/10 rounded-xl shadow-2xl max-w-2xl w-full p-8">
          ...
          <div className="mb-8">
            <h2 className="text-3xl font-serif font-bold text-mystic-text mb-2">
              {selectedEvent?.isCustom ? "Request a Private Session" : "Reserve Your Spot"}
            </h2>
            ...
            {selectedEvent?.isCustom && (
              <p className="text-mystic-muted text-xs mt-2">
                Please specify your preferred time in the notes section below
              </p>
            )}
          </div>
